...
cmd_help() {
    ...
    echo ""
    echo "── Status / 查看状态 ───────────────────────────────"
    echo "  /sounds:cs               Show config (theme, enabled, hooks) / 查看当前配置"
    echo "  /sounds:cs hook status   Show each hook's state / 查看每个 hook 的启用状态"
    echo ""
    echo "── Enable / Disable / 启用禁用 ─────────────────────"
    echo "  /sounds:cs enable        Enable plugin / 启用插件"
    echo "  /sounds:cs disable       Disable plugin (mute all) / 禁用插件（静音）"
    echo ""
    echo "── Hooks / 管理 Hooks ──────────────────────────────"
    echo "  Hook names / hook 名称："
    ...
    echo "  /sounds:cs theme store install <name>   Download & install / 下载并安装主题"
    echo "  /sounds:cs theme store preview <name>   Preview before install / 试听后决定是否安装"
    echo "  /sounds:cs theme store update [name]    Update installed themes / 更新已安装主题"
    echo ""
    echo "── Themes / 主题管理 ───────────────────────────────"
    echo "  /sounds:cs theme <name>              Switch global theme / 切换全局主题"
    echo "  /sounds:cs theme list                List installed themes / 查看所有已安装主题"
    echo "  /sounds:cs theme install <file>      Install .cstheme file / 安装主题包"
    echo "  /sounds:cs theme remove  <name>      Remove a theme / 删除已安装主题"
    echo "  /sounds:cs theme cache-clear         Clear audio cache / 清除缓存"
    echo ""
    echo "── Create a Theme / 制作本地主题 ───────────────────"
    echo "  1. Create a folder with 4 MP3s + manifest.json / 新建文件夹，放入 4 个 MP3 + manifest.json："
    echo "     mytheme/"
    ...
    echo "  3. Install & activate / 安装并切换："
    echo "     /sounds:cs theme install ./mytheme.cstheme"
    echo "     /sounds:cs theme mytheme"
    echo ""
    echo "── Per-project Config / 按项目配置 ─────────────────"
    echo "  Run from project root; only affects that project."
    echo "  在项目根目录运行，只影响当前项目："
    echo "  /sounds:cs project theme cyberpunk      Set project theme / 设置项目主题"
    echo "  /sounds:cs project hook enable  stop    Enable stop here / 项目内开启 stop"
    echo "  /sounds:cs project hook disable stop    Disable stop here / 项目内关闭 stop"
    echo "  /sounds:cs project hook status          Show project hooks / 查看项目 hook 配置"
    echo "  /sounds:cs project status               Show project config / 查看完整项目配置"
    echo "  /sounds:cs project clear                Reset to global / 清除项目配置"
    echo ""
    echo "  Config file: <project-root>/.claude/sounds.json"
    echo "  Priority: project > global > built-in default"
    echo "  优先级：项目配置 > 全局配置 > 内置默认"
    echo ""
    echo "── Web UI / 图形界面（推荐）────────────────────────"
    echo "  /sounds:cs ui            Launch Web UI, opens browser / 启动 Web UI，自动打开浏览器"
    echo "  /sounds:cs ui 8080       Use custom port / 指定端口启动"
    echo "  Features: toggle hooks, preview/switch themes, store, per-project config"
    echo "  功能：开关插件/Hooks、主题试听切换、商店安装、项目配置管理"
    echo ""
    echo "── Test Sounds / 测试声音 ──────────────────────────"
    echo "  /sounds:cs test              Play all sounds / 依次测试所有声音"
    echo "  /sounds:cs test stop         Test stop / 只测试 stop 声音"
    echo "  /sounds:cs test notification Test notification / 只测试 notification 声音"
    echo "  /sounds:cs test error        Test error / 只测试 error 声音"
    echo "  /sounds:cs test permission   Test permission / 只测试 permission 声音"
}
